...
class EmailReq extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		cek_nologin();
		$this->load->model('M_EmailReq');
	...
	public function index()
	{
		...
		$data = array(
			'title'        => "ICT Helpdesk - Email Request",
			'row'        => $this->M_EmailReq->getDataTicket($tahun),
			'rowtix'        => $this->M_EmailReq->getNoTicket($tahun),
			// 'rowuser'	=> $this->M_Troubleshoot->getUser()
		);
		$this->template->load('template', 'ER/dashboardER', $data);
	}
	...
	public function updateTask()
	{
		...
		$this->M_EmailReq->updateTask('tb_helpdesk', $data, $where);
		if ($this->db->affected_rows()) {
			$this->M_EmailReq->addHistoryHelpdesk($datahist, 'tb_helpdesk_history');
			if ($status == "PROCESSING") {
	...
	public function proposeRF()
	{
		...
		$this->M_EmailReq->proposeRF($dataBa, 'tb_berita_acara');
		$link = explode("/", $noba);
		if ($this->db->affected_rows()) {
			$this->M_EmailReq->updateTask('tb_helpdesk', $data, $where);
			$this->session->set_flashdata('flash', 'complete');
	...
	public function getNoBeritaAcara()
	{
		...
		$kodebaru = sprintf("%04s", $nourut) . '/' . 'ICT-ERF' . '/' . $labeltahun;
		echo $kodebaru;
	...
	public function searchTix_Cat()
	{
		...
		$data = array(
			...
			'row'		=> $this->M_EmailReq->getDataTicketFiltered($daterange[0], $akhir, $category),
			'rowtix'	=> $this->M_EmailReq->getNoTicketFiltered($daterange[0], $akhir, $category),
			'category'	=> $category,
			'awal' => $daterange[0],
			'akhir'	=> $akhir,
			// 'rowuser'	=> $this->M_Troubleshoot->getUser()
		);
		$this->template->load('template', 'ER/dashboardER-filtered', $data);
	}
