...
class AssetReq extends CI_Controller
{
	public function __construct()
	{
		parent::__construct();
		cek_nologin();
		$this->load->model('M_AssetReq');
	...
	public function index()
	{
		...
		$data = array(
			'title'        => "ICT Helpdesk - Asset Request",
			'row'        => $this->M_AssetReq->getDataTicket($tahun),
			'rowtix'        => $this->M_AssetReq->getNoTicket($tahun),
			// 'rowuser'	=> $this->M_Troubleshoot->getUser()
		);
		$this->template->load('template', 'AR/dashboardAR', $data);
	}
	...
	public function updateTask()
	{
		...
		$noticket     = $this->input->post('noticket', TRUE);
		$info         = $this->input->post('notes', TRUE);
		...
		$this->M_AssetReq->updateTask('tb_helpdesk', $data, $where);
		if ($this->db->affected_rows()) {
			$this->M_AssetReq->addHistoryHelpdesk($datahist, 'tb_helpdesk_history');
			if ($status == "PROCESSING") {

				$this->send($email, $nama, $noticket, $step, $information, $today, $tipecomplain, $user); // Eksekusi Fungsi kirim email   
			} else {
				$this->send2($email, $nama, $noticket, $step, $information, $today, $tipecomplain, $user); // Eksekusi Fungsi kirim email   
	...
	public function proposeRF()
	{
		...
		$this->M_AssetReq->proposeRF($dataBa, 'tb_berita_acara');
		$link = explode("/", $noba);
		if ($this->db->affected_rows()) {
			$this->M_AssetReq->updateTask('tb_helpdesk', $data, $where);
			$this->session->set_flashdata('flash', 'complete');
	...
	public function getNoBeritaAcara()
	{
		...
		$kodebaru = sprintf("%04s", $nourut) . '/' . 'ICT-ARF' . '/' . $labeltahun;
		echo $kodebaru;
	...
	public function searchTix_Cat()
	{
		...
		$data = array(
			'title'		=> "ICT Helpdesk - Asset Request",
			'row'		=> $this->M_AssetReq->getDataTicketFiltered($daterange[0], $akhir, $category),
			'rowtix'	=> $this->M_AssetReq->getNoTicketFiltered($daterange[0], $akhir, $category),
			'category'	=> $category,
			'awal' => $daterange[0],
			'akhir'	=> $akhir,
			// 'rowuser'	=> $this->M_Troubleshoot->getUser()
		);
		$this->template->load('template', 'AR/dashboardAR-filtered', $data);
	}


	// // EMAIL NOTIF UNTUK IN PROGRESS
	public function send($email, $nama, $noticket, $step, $information, $today, $tipecomplain, $user)
	{
		...
		$this->email->to($email);
		$this->email->cc($user);
		$this->email->subject('Notification Ticket Submitted No. ' . $noticket);
		$this->email->message('
		<html>
	...
	public function send2($email, $nama, $noticket, $step, $information, $today, $tipecomplain, $user)
	{

		if ($tipecomplain == "TS") {
			$isiemail = "Request Detail :" . $information;
